...
public class Main { //départ Jaune/vert
	...
	public static void suivreLigne(int n) { // suit la ligne de la couleur d'indice n 
		int i = 1;
		int p = 0;
		int q = 0;
		int r = 1;
	...
	public static void allerA(float x, float y) {
		pp = chassis.getPoseProvider();
		pose = pp.getPose();
		location = pose.getLocation();
		float posX1 = location.x;
	...
	public static void main(String[] args) {
		...
		avance(100);
		rotation(200,-90);
		suivreLigne(2); // s'aligne sur ligne bleue
		AuntilDetec();
		rotation(100,-90);
		chassis.stop();
		depose();
		...
		avance(100);
		rotation(100,90);
		suivreLigne(5); // suit la ligne rouge
		int A = ADouL(3,1,2);
		rotation(150,180);
		avanceLigne(A);
		chassis.closePliers();
